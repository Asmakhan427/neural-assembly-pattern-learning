...
<style>
    ...
    /* All text */
    .stMarkdown, p, label, div, span, h1, h2, h3, h4, h5, h6 {
        color: #e0e8f0 !important;
    }
    
    h1, h2, h3 { color: #4a9eff !important; }
    .stSidebar .stMarkdown, .stSidebar p, .stSidebar label { color: #c0d0e0 !important; }
    ...
    ::-webkit-scrollbar-thumb { background: #1a3a6a; border-radius: 5px; }
...
def render_sidebar() -> dict:
    ...
    plasticity_rate = st.sidebar.slider(
        "Plasticity rate (beta)", min_value=0.01, max_value=0.5,
    ...
    try:
        ...
        if params["dataset_type"] == "synthetic":
            ...
            if patterns is not None and len(patterns) > 0:
                st.success(f"Loaded {len(patterns)} digits from MNIST")
            else:
                st.warning("MNIST download failed. Using synthetic patterns as fallback.")
                patterns = simulator.generate_patterns(
                    n_patterns=params["n_patterns"], 
                    pattern_size=params["pattern_size"]
                )
            ...
            if patterns is not None and len(patterns) > 0:
                st.success(f"Loaded {len(patterns)} items from Fashion-MNIST")
            else:
                st.warning("Fashion-MNIST download failed. Using synthetic patterns as fallback.")
                patterns = simulator.generate_patterns(
                    n_patterns=params["n_patterns"], 
                    pattern_size=params["pattern_size"]
                )
            ...
            st.success(f"Loaded {len(image_paths)} custom images")
...
def action_train(n_rounds: int) -> None:
    ...
    if simulator is None or st.session_state.patterns is None:
        st.warning("Please initialize the brain and load data first.")
        return
...
def action_test(occlusion: float, pattern_idx: int) -> None:
    ...
    if simulator is None or not st.session_state.assemblies:
        st.warning("Please initialize and train the brain first.")
        return
...
def main() -> None:
    ...
    <div style="text-align: center; padding: 20px 0; background: transparent;">
        ...
        <p style="color: #80a0c0; font-size: 1.2rem; margin-top: 10px;">
            Pattern recognition using Assembly Calculus with MNIST, Fashion-MNIST, and custom images
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    tab_learn, tab_test, tab_visualize, tab_about = st.tabs(["Learn", "Test", "Visualize", "About"])
                ...
                for i in range(patterns.shape[0]):
                    with cols[i % len(cols)]:
                        ...
                        try:
                            ...
                            st.error(f"Error displaying pattern {i}")
            else:
                st.warning("No patterns loaded. Try initializing again.")
            ...
            if st.session_state.assemblies is None:
                ...
                with left:
                    ...
                    fig = plot_assembly_network(
                        ...
                        area_label="Final Area (Pattern 0)",
                    )
    ...
    with tab_test:
        if not st.session_state.assemblies:
            ...
            if st.button("Run Recognition and Completion", use_container_width=True):
            ...
            if st.session_state.results is not None:
                ...
                if r["best_label"] is None:
                    st.warning("No confident match found. Try lowering the occlusion level.")
                elif correct:
    ...
    with tab_about:
        ...
        <h4 style="color: #6ab0ff;">How it works:</h4>
